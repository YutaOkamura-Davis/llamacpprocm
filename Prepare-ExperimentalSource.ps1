...
function Initialize-PinnedClone {
    ...
    if (-not (Test-Path -LiteralPath $path)) {
        Invoke-Checked git @('clone', '--filter=blob:none', '--no-checkout', $Repository, $path)
    }
    ...
    if ($dirty) { throw "$path has local changes. Use a clean workspace or -ForceRefresh." }
...
$dflashPath = Initialize-PinnedClone 'dflash-base' $DFlashRepo $DFlashCommit
$donorPath = Initialize-PinnedClone 'rocmfpx-donor' $RocmFpxRepo $RocmFpxCommit
$hybridPath = Join-Path $workspaceFull 'hybrid'

if (-not (Test-Path -LiteralPath $hybridPath)) {
    Invoke-Checked git @('clone', '--no-hardlinks', $dflashPath, $hybridPath)
    Invoke-Checked git @('-C', $hybridPath, 'remote', 'rename', 'origin', 'dflash-local')
    Invoke-Checked git @('-C', $hybridPath, 'remote', 'add', 'dflash-upstream', $DFlashRepo)
    Invoke-Checked git @('-C', $hybridPath, 'remote', 'add', 'rocmfpx-donor', $RocmFpxRepo)
    Invoke-Checked git @('-C', $hybridPath, 'checkout', '-B', 'win-rocmfpx-experiment', $DFlashCommit)
}

$manifest = [ordered]@{
    CreatedAt = (Get-Date).ToString('o')
    Workspace = $workspaceFull
    DFlash = [ordered]@{ Repository = $DFlashRepo; Commit = $DFlashCommit; Path = $dflashPath }
    RocmFPX = [ordered]@{ Repository = $RocmFpxRepo; Commit = $RocmFpxCommit; Path = $donorPath }
    Hybrid = [ordered]@{ Path = $hybridPath; BaseCommit = $DFlashCommit; Branch = 'win-rocmfpx-experiment' }
}
